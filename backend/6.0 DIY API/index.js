...
app.get("/filter",(req,res)=>
{
  ...
  if(!type){
    return res.status(400).json({error:"invalild input"});
  }
  const filterd=jokes.filter((joke)=>joke.jokeType.toLowerCase()===type.toLowerCase());
  res.json(filterd);
}
)
//4. POST a new joke
...
app.put("/jokes/:id", (req, res) => {
  ...
  const update={
    id:id,
    jokeText:req.body.jokeText || "NO TEXT PASSED",
    jokeType: req.body.jokeType || "NO TYPE PASSED",
  };
